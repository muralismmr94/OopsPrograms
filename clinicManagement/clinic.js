...
class clinic {
     ...
     * @param {JsonObject} clinicJsonObject as taken parameter
     */
    getNameFromDoctor(clinicJsonObject) {
        var nameArray = [];
        var data = clinicJsonObject.doctors;
        /**
    ...
    getSpecialisationFromDoctor(clinicJsonObject) {
        ...
        for (let key in data) {
             ...
             */
            nameArray.push(data[key].specialization);
        }
        return nameArray;
    }
    /**
    ...
    getNameFromPatient(clinicJsonObject) {
        var nameArray = [];
        var data = clinicJsonObject.patients;
        /**
    ...
    getPhoneNumberFromPatient(clinicJsonObject) {
         ...
         */
        for (let key in data) {
            nameArray.push(data[key].phoneNo);
        }
        return nameArray;
    }
    /**
    ...
    clinicManagement(clinicJsonObject) {
        try {
            var doctor = clinicJsonObject.doctors;
            var patient = clinicJsonObject.patients;
            ...
            if (choice == 1) {
                ...
                if (choice == 1) {
                    console.log("Doctors available - " + this.getNameFromDoctor(clinicJsonObject));
                     ...
                     */
                    console.log("ID of the doctors are - " + this.getIdFromDoctor(clinicJsonObject));
                    var docID = readline.questionInt("\nEnter the id of the doctor: ");
                    /**
                     * Validation to accept correct id
                     */
                    while (!this.getIdFromDoctor(clinicJsonObject).includes(docID)) {
                        console.log("Enter the valid Id");
                        docID = readline.questionInt("Enter the id of the doctor: ");
                    }
                    /**
                     ...
                     */
                    console.log("Specialisation of the doctors - " +
                     ...
                     */
                    while (!this.getSpecialisationFromDoctor(clinicJsonObject).includes(specialisation)) {
                ...
                if (choice == 1) {
                    console.log("\nName of the patients are - " + this.getNameFromPatient(clinicJsonObject));
                    var patientName = readline.question("\nEnter the name of the Patient: ");
                    /**
                     * Validation to accept valid name
                     */
                    while (!this.getNameFromPatient(clinicJsonObject).includes(patientName)) {
                    ...
                    console.log("\nPhonenumber of the patients are - " + this.getPhoneNumberFromPatient(clinicJsonObject));
                    var phoneNumber = readline.questionInt("\nEnter the phoneNumber of the patient");
                    /**
                     * Validation to accept valid phone number
                     */
                    while (!this.getPhoneNumberFromPatient(clinicJsonObject).includes(phoneNumber)) {
                        console.log("Enter the valid phonenum");
                        phoneNumber = readline.questionInt("Enter the phoneNumber of the patient");
                    }
                    /**
                     * Display the patient details for given phone number
                     */
                    for (let key in patient) {
                        if (patient[key].phoneNo == phoneNumber) {
                ...
                var appointmentFrom = readline.question("Enter the doctor name to take appointment with: ");
                /**
